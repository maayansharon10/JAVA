...
class Book {
    ...
    /** The book's place on shelf. negative if not on shelf, else - it's place on shelf */
    int bookId = -1;
    ...
    int getLiteraryValue(){
        int value = comicValue + dramaticValue + educationalValue;
        return value;
    }

    /**
     * Sets the given id as the id of the current borrower of this book,
     * -1 if no patron is currently borrowin it.
     * @param borrowerId borrowerdId
     */
    ...
    /**
     * set place on shelf
     * @param place - represents place on shelf(library) - non- negative int.
     *             negative - if book not in any library.
     */
    void setBookId(int place){
        bookId = place;
    }
